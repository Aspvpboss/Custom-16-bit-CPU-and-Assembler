...
int exe_add(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->alu.add(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.add(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_sub(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.sub(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.sub(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_mul(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.mul(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.mul(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_div(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.div(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.div(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_and(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.and(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr]);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.and(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr]);
        return 0;
...
int exe_nor(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.nor(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr]);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.nor(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr]);
        return 0;
...
int exe_xor(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.xor(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr]);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.xor(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr]);
        return 0;
...
int exe_ars(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.ars(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.ars(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_lrs(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.lrs(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.lrs(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_lls(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){        
        ...
        emu->alu.lls(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_EIGHT || addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->alu.lls(registers[operands[OP_ZERO]], operands[OP_TWO], &registers[write_reg_addr], &emu->flags);
        return 0;
...
int exe_fadd(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.add(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->fpu.add(registers[operands[OP_ZERO]], (*(f16*)&operands[OP_TWO]), &registers[write_reg_addr], &emu->flags);
        return 0;
    }
...
int exe_fsub(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.sub(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->fpu.sub(registers[operands[OP_ZERO]], (*(f16*)&operands[OP_TWO]), &registers[write_reg_addr], &emu->flags);
        return 0;
    }
...
int exe_fmul(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.mul(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->fpu.mul(registers[operands[OP_ZERO]], (*(f16*)&operands[OP_TWO]), &registers[write_reg_addr], &emu->flags);
        return 0;
    }
...
int exe_fdiv(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.div(registers[operands[OP_ZERO]], registers[operands[OP_TWO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->fpu.div(registers[operands[OP_ZERO]], (*(f16*)&operands[OP_TWO]), &registers[write_reg_addr], &emu->flags);
        return 0;
    }
...
int exe_fsqrt(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.sqrt(registers[operands[OP_ZERO]], &registers[write_reg_addr], &emu->flags);
        return 0;

    } else if(addr_mode == ADDR_REG_IMMEDIATE_SIXTEEN){

        emu->fpu.sqrt((*(f16*)&operands[OP_TWO]), &registers[write_reg_addr], &emu->flags);
        return 0;
    }
...
int exe_fint(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.fint(float_registers[operands[OP_ZERO]], &alu_registers[write_reg_addr]);
        return 0;
...
int exe_iflo(Emulator *emu, EMU_Decoded_Instruction *instruction){
    ...
    if(addr_mode == ADDR_REG){
        ...
        emu->fpu.iflo(alu_registers[operands[OP_ZERO]], &float_registers[write_reg_addr]);
        return 0;
